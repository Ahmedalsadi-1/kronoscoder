...
- Add a guarded e2e route in backend (enabled only when a dedicated env flag is set by e2e-local runner).
  - New route file: `packages/kronoscode/src/server/routes/e2e.ts`
  - Mount from: `packages/kronoscode/src/server/server.ts`
  - Gate behind env flag (for example `KRONOSCODE_E2E=1`) so this route is never exposed in normal runs.
