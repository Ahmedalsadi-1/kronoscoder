...
const stateFile = path.join(desktopDir, '.kronoscode-cli-state.json');
...
const CLI_ARGS_ENV =
  process.env.KRONOSCHAMBER_KRONOSCODE_ARGS;
...
async function resolveCliPath() {
  ...
  for (const segment of envPath.split(path.delimiter)) {
    const candidate = path.join(segment, 'kronoscode');
  ...
  throw new Error(
    'Unable to locate the KronosCode CLI. Set KRONOSCHAMBER_KRONOSCODE_PATH to the executable.',
  );
}
...
function spawnCli(cliPath, args) {
  const env = {
    ...
    KRONOSCHAMBER_KRONOSCODE_PORT:
      ...
      process.env.KRONOSCHAMBER_INTERNAL_PORT ||
      process.env.OPENCHAMBER_INTERNAL_PORT ||
      '0',
  };
  const cwd =
    process.env.KRONOSCHAMBER_KRONOSCODE_CWD ||
    process.cwd();
...
export async function startCli({ silent = false } = {}) {
  ...
  if (existing?.pid && isProcessAlive(existing.pid)) {
    if (!silent) {
      console.log(`[desktop:start-cli] KronosCode CLI already running (pid ${existing.pid}).`);
    }
    return existing.pid;
  }
  ...
  if (!silent) {
    console.log(`[desktop:start-cli] KronosCode CLI started (${cliPath}) pid ${child.pid}.`);
  }
  return child.pid;
}
...
export async function stopCli({ silent = false } = {}) {
  ...
  if (!state?.pid) {
    if (!silent) {
      console.log('[desktop:stop-cli] No KronosCode CLI PID recorded.');
    }
    return;
  }
  ...
  while (Date.now() - start < timeoutMs) {
    if (!isProcessAlive(pid)) {
      await removeStateFile();
      if (!silent) {
        console.log('[desktop:stop-cli] KronosCode CLI stopped.');
      }
      return;
    }
...
async function main() {
  ...
  if (!command || command === '--help' || command === '-h') {
    console.log(`Usage: node ${path.basename(process.argv[1] || 'kronoscode-cli.mjs')} <start|stop|status>`);
    process.exit(0);
  }
  ...
  if (command === 'status') {
    ...
    if (state?.pid && isProcessAlive(state.pid)) {
      console.log(`KronosCode CLI running (pid ${state.pid}).`);
    } else {
      console.log('KronosCode CLI not running.');
    }
    process.exit(0);
    return;
  }
...
if (import.meta.url === pathToFileURL(process.argv[1] || '').href) {
  main().catch((error) => {
    console.error('[desktop:kronoscode-cli] Unexpected error:', error);
    process.exit(1);
  });
}
