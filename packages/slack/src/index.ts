...
import { createOpencode, type ToolPart } from "@kronoscode-ai/sdk"
...
console.log("🚀 Starting kronoscode server...")
const kronoscode = await createOpencode({
  port: 0,
})
...
;(async () => {
  const events = await kronoscode.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new kronoscode session...")
    const { client, server } = kronoscode
    ...
    console.log("✅ Created kronoscode session:", createResult.data.id)
  ...
  console.log("📝 Sending to kronoscode:", message.text)
