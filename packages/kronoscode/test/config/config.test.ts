...
const managedConfigDir = process.env.KRONOSCODE_TEST_MANAGED_CONFIG_DIR!

afterEach(async () => {
  await fs.rm(managedConfigDir, { force: true, recursive: true }).catch(() => {})
})

async function writeManagedSettings(settings: object, filename = "kronoscode.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Filesystem.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "kronoscode.json") {
  ...
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(
        path.join(dir, "kronoscode.jsonc"),
        `{
        // This is a comment
        "$schema": "https://kronoscode.ai/config.json",
        "model": "test/model",
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Filesystem.readText(path.join(tmp.path, "kronoscode.json"))
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "kronoscode.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .kronoscode directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const kronoscodeDir = path.join(dir, ".kronoscode")
      await fs.mkdir(kronoscodeDir, { recursive: true })
      const agentDir = path.join(kronoscodeDir, "agent")
...
test("loads agents from .kronoscode/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const kronoscodeDir = path.join(dir, ".kronoscode")
      await fs.mkdir(kronoscodeDir, { recursive: true })

      const agentsDir = path.join(kronoscodeDir, "agents")
...
test("loads commands from .kronoscode/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const kronoscodeDir = path.join(dir, ".kronoscode")
      await fs.mkdir(kronoscodeDir, { recursive: true })

      const commandDir = path.join(kronoscodeDir, "command")
...
test("loads commands from .kronoscode/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const kronoscodeDir = path.join(dir, ".kronoscode")
      await fs.mkdir(kronoscodeDir, { recursive: true })

      const commandsDir = path.join(kronoscodeDir, "commands")
...
test("does not try to install dependencies in read-only KRONOSCODE_CONFIG_DIR", async () => {
  ...
  const prev = process.env.KRONOSCODE_CONFIG_DIR
  process.env.KRONOSCODE_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.KRONOSCODE_CONFIG_DIR
    else process.env.KRONOSCODE_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable KRONOSCODE_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Filesystem.write(
        path.join(dir, "kronoscode.json"),
        JSON.stringify({ $schema: "https://kronoscode.ai/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const baseUrl = pathToFileURL(path.join(tmp.path, "kronoscode.json")).href
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .kronoscode config
      const projectDir = path.join(dir, "project")
      const kronoscodeDir = path.join(projectDir, ".kronoscode")
      await fs.mkdir(kronoscodeDir, { recursive: true })
      ...
      // Local .kronoscode config with different plugins
      await Filesystem.write(
        path.join(kronoscodeDir, "kronoscode.json"),
        JSON.stringify({
          $schema: "https://kronoscode.ai/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .kronoscode config with some overlapping plugins
      await Filesystem.write(
        path.join(kronoscodeDir, "kronoscode.json"),
        JSON.stringify({
          $schema: "https://kronoscode.ai/config.json",
...
// Note: preload.ts sets KRONOSCODE_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("local .kronoscode config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .kronoscode directory config enables it
      const kronoscodeDir = path.join(dir, ".kronoscode")
      await fs.mkdir(kronoscodeDir, { recursive: true })
      await Filesystem.write(
        path.join(kronoscodeDir, "kronoscode.json"),
...
test("project config overrides remote well-known config", async () => {
  ...
  const mockFetch = mock((url: string | URL | Request) => {
    ...
    if (urlStr.includes(".well-known/kronoscode")) {
      fetchedUrl = urlStr
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        expect(fetchedUrl).toBe("https://example.com/.well-known/kronoscode")
...
describe("getPluginName", () => {
  ...
  test("extracts name from npm package with version", () => {
    expect(Config.getPluginName("oh-my-kronoscode@2.4.3")).toBe("oh-my-kronoscode")
  ...
  test("extracts name from scoped npm package", () => {
    ...
    expect(Config.getPluginName("@kronoscode/plugin@2.0.0")).toBe("@kronoscode/plugin")
  })
...
describe("deduplicatePlugins", () => {
  ...
  test("prefers local file over npm package with same name", () => {
    const plugins = ["oh-my-kronoscode@2.4.3", "file:///project/.kronoscode/plugin/oh-my-kronoscode.js"]

    const result = Config.deduplicatePlugins(plugins)

    expect(result.length).toBe(1)
    expect(result[0]).toBe("file:///project/.kronoscode/plugin/oh-my-kronoscode.js")
  })
  ...
  test("local plugin directory overrides global kronoscode.json plugin", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const kronoscodeDir = path.join(projectDir, ".kronoscode")
        const pluginDir = path.join(kronoscodeDir, "plugin")
        await fs.mkdir(pluginDir, { recursive: true })
...
describe("KRONOSCODE_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["KRONOSCODE_DISABLE_PROJECT_CONFIG"]
    process.env["KRONOSCODE_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["KRONOSCODE_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["KRONOSCODE_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .kronoscode/ directories when flag is set", async () => {
    const originalEnv = process.env["KRONOSCODE_DISABLE_PROJECT_CONFIG"]
    process.env["KRONOSCODE_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .kronoscode directory with a command
          const kronoscodeDir = path.join(dir, ".kronoscode", "command")
          await fs.mkdir(kronoscodeDir, { recursive: true })
          await Filesystem.write(path.join(kronoscodeDir, "test-cmd.md"), "# Test Command\nThis is a test command.")
        },
      })
      await Instance.provide({
        directory: tmp.path,
        fn: async () => {
          ...
          // Project .kronoscode should NOT be in directories list
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["KRONOSCODE_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["KRONOSCODE_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["KRONOSCODE_CONFIG_DIR"]
      process.env["KRONOSCODE_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["KRONOSCODE_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["KRONOSCODE_CONFIG_DIR"]
      } else {
        process.env["KRONOSCODE_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("KRONOSCODE_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["KRONOSCODE_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["KRONOSCODE_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["KRONOSCODE_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["KRONOSCODE_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await Config.get()
          // Should load from KRONOSCODE_CONFIG_DIR, not project
...
describe("KRONOSCODE_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in KRONOSCODE_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["KRONOSCODE_CONFIG_CONTENT"]
    const originalTestVar = process.env["TEST_CONFIG_VAR"]
    process.env["TEST_CONFIG_VAR"] = "test_api_key_12345"
    process.env["KRONOSCODE_CONFIG_CONTENT"] = JSON.stringify({
      $schema: "https://kronoscode.ai/config.json",
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["KRONOSCODE_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["KRONOSCODE_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in KRONOSCODE_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["KRONOSCODE_CONFIG_CONTENT"]
