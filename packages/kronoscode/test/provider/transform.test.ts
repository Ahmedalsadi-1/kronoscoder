...
describe("ProviderTransform.message - strip openai metadata when store=false", () => {
  ...
  test("preserves metadata using providerID key when store is false", () => {
    const kronoscodeModel = {
      ...openaiModel,
      providerID: "kronoscode",
      api: {
        id: "kronoscode-test",
        url: "https://api.kronoscode.ai",
        npm: "@ai-sdk/openai-compatible",
      },
    }
    const msgs = [
    ...
    const result = ProviderTransform.message(msgs, kronoscodeModel, { store: false }) as any[]

    expect(result[0].content[0].providerOptions?.kronoscode?.itemId).toBe("msg_123")
    expect(result[0].content[0].providerOptions?.kronoscode?.otherOption).toBe("value")
  })
  ...
  test("preserves itemId across all providerOptions keys", () => {
    ...
    const msgs = [
      {
        ...
        providerOptions: {
          ...
          kronoscode: { itemId: "msg_kronoscode" },
        ...
        content: [
          {
            ...
            providerOptions: {
              ...
              kronoscode: { itemId: "msg_kronoscode_part" },
    ...
    expect(result[0].providerOptions?.kronoscode?.itemId).toBe("msg_kronoscode")
    expect(result[0].providerOptions?.extra?.itemId).toBe("msg_extra")
    expect(result[0].content[0].providerOptions?.openai?.itemId).toBe("msg_openai_part")
    expect(result[0].content[0].providerOptions?.kronoscode?.itemId).toBe("msg_kronoscode_part")
