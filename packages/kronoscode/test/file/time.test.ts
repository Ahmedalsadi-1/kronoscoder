...
describe("file/time", () => {
  ...
  describe("assert()", () => {
    ...
    test("skips check when KRONOSCODE_DISABLE_FILETIME_CHECK is true", async () => {
      await using tmp = await tmpdir()
      const filepath = path.join(tmp.path, "file.txt")
      await fs.writeFile(filepath, "content", "utf-8")
      ...
      await Instance.provide({
        ...
        fn: async () => {
          ...
          const original = Flag.KRONOSCODE_DISABLE_FILETIME_CHECK
          ;(Flag as { KRONOSCODE_DISABLE_FILETIME_CHECK: boolean }).KRONOSCODE_DISABLE_FILETIME_CHECK = true

          try {
            // Should not throw even though file wasn't read
            await FileTime.assert(sessionID, filepath)
          } finally {
            ;(Flag as { KRONOSCODE_DISABLE_FILETIME_CHECK: boolean }).KRONOSCODE_DISABLE_FILETIME_CHECK = original
          }
        },
      })
    })
