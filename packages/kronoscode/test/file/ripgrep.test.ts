  ...
  test("defaults to include hidden", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        ...
        await fs.mkdir(path.join(dir, ".kronoscode"), { recursive: true })
        await Bun.write(path.join(dir, ".kronoscode", "thing.json"), "{}")
      },
    })

    const files = await Array.fromAsync(Ripgrep.files({ cwd: tmp.path }))
    const hasVisible = files.includes("visible.txt")
    const hasHidden = files.includes(path.join(".kronoscode", "thing.json"))
