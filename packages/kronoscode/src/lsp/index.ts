...
export namespace LSP {
  ...
  export const Event = {
    ...
    Diagnostics: BusEvent.define("lsp.diagnostics", z.object({
      diagnostics: z.record(z.string(), z.array(z.any()))
    })),
  }
  ...
  const filterExperimentalServers = (servers: Record<string, LSPServer.Info>) => {
    if (Flag.KRONOSCODE_EXPERIMENTAL_LSP_TY) {
      // If experimental flag is enabled, disable pyright
      if (servers["pyright"]) {
        log.info("LSP server pyright is disabled because KRONOSCODE_EXPERIMENTAL_LSP_TY is enabled")
