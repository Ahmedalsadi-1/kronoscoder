...
export namespace Provider {
  ...
  const CUSTOM_LOADERS: Record<string, CustomLoader> = {
    ...
    async kronoscode(input) {
      const hasKey = await (async () => {
        const env = Env.all()
        if (input.env.some((item) => env[item])) return true
        if (await Auth.get(input.id)) return true
        const config = await Config.get()
        if (config.provider?.["kronoscode"]?.options?.apiKey) return true
        return false
      })()
    ...
    "amazon-bedrock": async () => {
      ...
      return {
        ...
        async getModel(sdk: any, modelID: string, options?: Record<string, any>) {
          ...
          // 1. options.region from kronoscode.json provider config
    ...
    gitlab: async (input) => {
      ...
      const aiGatewayHeaders = {
        "User-Agent": `kronoscode/${Installation.VERSION} gitlab-ai-provider/${GITLAB_PROVIDER_VERSION} (${os.platform()} ${os.release()}; ${os.arch()})`,
      ...
      if (!apiToken) {
        throw new Error(
          "CLOUDFLARE_API_TOKEN (or CF_AIG_TOKEN) is required for Cloudflare AI Gateway. " +
            "Set it via environment variable or run `kronoscode auth cloudflare-ai-gateway`.",
        )
      }
  ...
  const state = Instance.state(async () => {
    ...
    for (const [providerID, provider] of Object.entries(providers)) {
      ...
      for (const [modelID, model] of Object.entries(provider.models)) {
        ...
        if (model.status === "alpha" && !Flag.KRONOSCODE_ENABLE_EXPERIMENTAL_MODELS) delete provider.models[modelID]
  ...
  export async function getSmallModel(providerID: string) {
    ...
    if (provider) {
      ...
      if (providerID.startsWith("kronoscode")) {
        priority = ["gpt-5-nano"]
      }
    ...
    // Check if kronoscode provider is available before using it
    const kronoscodeProvider = await state().then((state) => state.providers["kronoscode"])
    if (kronoscodeProvider && kronoscodeProvider.models["gpt-5-nano"]) {
      return getModel("kronoscode", "gpt-5-nano")
    }
