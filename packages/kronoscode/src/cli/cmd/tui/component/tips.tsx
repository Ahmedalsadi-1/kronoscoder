...
const TIPS = [
  ...
  "Run {highlight}/share{/highlight} to create a public link to your conversation at kronoscode.ai",
  ...
  "Create {highlight}kronoscode.json{/highlight} in project root for project-specific settings",
  "Place settings in {highlight}~/.config/kronoscode/kronoscode.json{/highlight} for global config",
  ...
  "KronosCode auto-handles OAuth for remote MCP servers requiring auth",
  "Add {highlight}.md{/highlight} files to {highlight}.kronoscode/command/{/highlight} to define reusable custom prompts",
  "Use {highlight}$ARGUMENTS{/highlight}, {highlight}$1{/highlight}, {highlight}$2{/highlight} in custom commands for dynamic input",
  "Use backticks in commands to inject shell output (e.g., {highlight}`git status`{/highlight})",
  "Add {highlight}.md{/highlight} files to {highlight}.kronoscode/agent/{/highlight} for specialized AI personas",
  "Configure per-agent permissions for {highlight}edit{/highlight}, {highlight}bash{/highlight}, and {highlight}webfetch{/highlight} tools",
  'Use patterns like {highlight}"git *": "allow"{/highlight} for granular bash permissions',
  'Set {highlight}"rm -rf *": "deny"{/highlight} to block destructive commands',
  'Configure {highlight}"git push": "ask"{/highlight} to require approval before pushing',
  "KronosCode auto-formats files using prettier, gofmt, ruff, and more",
  'Set {highlight}"formatter": false{/highlight} in config to disable all auto-formatting',
  "Define custom formatter commands with file extensions in config",
  "KronosCode uses LSP servers for intelligent code analysis",
  "Create {highlight}.ts{/highlight} files in {highlight}.kronoscode/tools/{/highlight} to define new LLM tools",
  "Tool definitions can invoke scripts written in Python, Go, etc",
  "Add {highlight}.ts{/highlight} files to {highlight}.kronoscode/plugin/{/highlight} for event hooks",
  "Use plugins to send OS notifications when sessions complete",
  "Create a plugin to prevent KronosCode from reading sensitive files",
  "Use {highlight}kronoscode run{/highlight} for non-interactive scripting",
  "Use {highlight}kronoscode --continue{/highlight} to resume the last session",
  "Use {highlight}kronoscode run -f file.ts{/highlight} to attach files via CLI",
  "Use {highlight}--format json{/highlight} for machine-readable output in scripts",
  "Run {highlight}kronoscode serve{/highlight} for headless API access to KronosCode",
  "Use {highlight}kronoscode run --attach{/highlight} to connect to a running server",
  "Run {highlight}kronoscode upgrade{/highlight} to update to the latest version",
  "Run {highlight}kronoscode auth list{/highlight} to see all configured providers",
  "Run {highlight}kronoscode agent create{/highlight} for guided agent creation",
  "Use {highlight}/kronoscode{/highlight} in GitHub issues/PRs to trigger AI actions",
  "Run {highlight}kronoscode github install{/highlight} to set up the GitHub workflow",
  "Comment {highlight}/kronoscode fix this{/highlight} on issues to auto-create PRs",
  "Comment {highlight}/oc{/highlight} on PR code lines for targeted code reviews",
  'Use {highlight}"theme": "system"{/highlight} to match your terminal\'s colors',
  "Create JSON theme files in {highlight}.kronoscode/themes/{/highlight} directory",
  ...
  "Run {highlight}kronoscode debug config{/highlight} to troubleshoot configuration",
  ...
  "Run {highlight}docker run -it --rm ghcr.io/anomalyco/kronoscode{/highlight} for containerized use",
  "Use {highlight}/connect{/highlight} with KronosCode Zen for curated, tested models",
