...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run kronoscode",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.exitCode === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for kronoscode session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found kronoscode session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await $`kronoscode import ${sessionUrl}`.nothrow()
        ...
        UI.println()
        UI.println("Starting kronoscode...")
        UI.println()

        // Launch kronoscode TUI with session ID if available
        const { spawn } = await import("child_process")
        const kronoscodeArgs = sessionId ? ["-s", sessionId] : []
        const kronoscodeProcess = spawn("kronoscode", kronoscodeArgs, {
          stdio: "inherit",
          cwd: process.cwd(),
        })

        await new Promise<void>((resolve, reject) => {
          kronoscodeProcess.on("exit", (code) => {
            if (code === 0) resolve()
            else reject(new Error(`kronoscode exited with code ${code}`))
          })
          kronoscodeProcess.on("error", reject)
        })
      },
    })
  },
