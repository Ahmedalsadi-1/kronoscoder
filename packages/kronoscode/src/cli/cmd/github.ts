...
const AGENT_USERNAME = "kronoscode-agent[bot]"
const AGENT_REACTION = "eyes"
const WORKFLOW_FILE = ".github/workflows/kronoscode.yml"
      ...
      async fn() {
        {
          ...
          function printNextSteps() {
            ...
            prompts.outro(
              [
                ...
                "",
                "   Learn more about the GitHub agent - https://kronoscode.ai/docs/github/#usage-examples",
              ].join("\n"),
            )
          }
          ...
          async function installGitHubApp() {
            ...
            // Open browser
            const url = "https://github.com/apps/kronoscode-agent"
            const command =
            ...
            async function getInstallation() {
              return await fetch(
                `https://api.kronoscode.ai/get_github_app_installation?owner=${app.owner}&repo=${app.repo}`,
              )
                .then((res) => res.json())
...
jobs:
  kronoscode:
    if: |
      ...
      startsWith(github.event.comment.body, '/kronoscode')
    ...
    steps:
      ...
      - name: Run kronoscode
        uses: anomalyco/kronoscode/github@latest${envStr}
        with:
...
export const GithubRunCommand = cmd({
  ...
  async handler(args) {
    await bootstrap(process.cwd(), async () => {
      ...
      const shareBaseUrl = isMock ? "https://dev.kronoscode.ai" : "https://kronoscode.ai"
      ...
      try {
        ...
        console.log("kronoscode session", session.id)
      ...
      function normalizeOidcBaseUrl(): string {
        ...
        if (!value) return "https://api.kronoscode.ai"
      ...
      async function getUserPrompt() {
        ...
        const mentions = (process.env["MENTIONS"] || "/kronoscode,/oc")
          .split(",")
      ...
      async function chat(message: string, files: PromptFiles = []) {
        console.log("Sending message to kronoscode...")
      ...
      async function getOidcToken() {
        try {
          return await core.getIDToken("kronoscode-github-action")
        } catch (error) {
      ...
      function generateBranchName(type: "issue" | "pr" | "schedule" | "dispatch") {
        ...
        if (type === "schedule" || type === "dispatch") {
          ...
          return `kronoscode/${type}-${hex}-${timestamp}`
        }
        return `kronoscode/${type}${issueId}-${timestamp}`
      }
...
Co-authored-by: ${actor} <${actor}@users.noreply.github.com>"`
      ...
      function footer(opts?: { image?: boolean }) {
        const image = (() => {
          ...
          return `<a href="${shareBaseUrl}/s/${shareId}"><img width="200" alt="${titleAlt}" src="https://social-cards.sst.dev/kronoscode-share/${title64}.png?model=${providerID}/${modelID}&version=${session.version}&id=${shareId}" /></a>\n`
        })()
        const shareUrl = shareId ? `[kronoscode session](${shareBaseUrl}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
    ...
    issue(number: $number) {
      ...
      function buildPromptDataForIssue(issue: GitHubIssue) {
        ...
        return [
          ...
          "- Git push and PR creation are handled AUTOMATICALLY by the kronoscode infrastructure after your response",
