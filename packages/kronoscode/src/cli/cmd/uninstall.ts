...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall kronoscode and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall KronosCode")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g kronoscode-ai",
      pnpm: "pnpm uninstall -g kronoscode-ai",
      bun: "bun remove -g kronoscode-ai",
      yarn: "yarn global remove kronoscode-ai",
      brew: "brew uninstall kronoscode",
      choco: "choco uninstall kronoscode",
      scoop: "scoop uninstall kronoscode",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result =
        method === "choco"
          ? await $`echo Y | choco uninstall kronoscode -y -r`.quiet().nothrow()
  ...
  UI.empty()
  prompts.log.success("Thank you for using KronosCode!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# kronoscode") || content.includes(".kronoscode/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".kronoscode/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".kronoscode/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".kronoscode"))
    ) {
      continue
    }
