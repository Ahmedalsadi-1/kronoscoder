...
const resolveBaseUrl = () => {
  const env =
    typeof process.env.KRONOSCHAMBER_SCREENPIPE_BASE_URL === "string"
...
const mapItem = (entry: any, index: number): ScreenpipeItem => {
  ...
  const id = content.frame_id ?? content.chunk_id ?? content.id ?? `${type}-${index}`
...
const queryScreenpipe = async (
  ...
  try {
    ...
    const pagination =
      (payload as any)?.pagination && typeof (payload as any).pagination === "object" ? (payload as any).pagination : {}
...
const availabilityMessage = () =>
  `Screenpipe is unavailable at ${resolveBaseUrl()}. Start Screenpipe locally to enable screen/audio capture.`

const WARNING_SHOWN_KEY = "kronoscode_screenpipe_warning_shown"

const shouldShowWarning = (): boolean => {
  if (typeof globalThis === "undefined") return true
  const shown = (globalThis as any)[WARNING_SHOWN_KEY]
  if (shown) return false
  ;(globalThis as any)[WARNING_SHOWN_KEY] = true
  return true
}

const FIRST_USE_WARNING = `
╔══════════════════════════════════════════════════════════════════╗
║              🔒 SCREENPIPE PERMISSION REQUIRED                 ║
╠══════════════════════════════════════════════════════════════════╣
║  Screenpipe needs to run to capture screen and audio.          ║
║                                                                  ║
║  To enable:                                                   ║
║    • Run: screenpipe --port 3030                              ║
║    • Or set: KRONOSCHAMBER_SCREENPIPE_BASE_URL               ║
║                                                                  ║
║  This is a one-time warning. The tool will work when         ║
...
export const ScreenpipeSearchTool = Tool.define("screenpipe_search", {
  ...
  async execute(params, ctx) {
    ...
    const showWarning = !available && shouldShowWarning()

    if (!available) {
      return {
        title: "Screenpipe unavailable",
        output: showWarning ? FIRST_USE_WARNING + "\n\n" + availabilityMessage() : availabilityMessage(),
...
export const ScreenpipeRecallTool = Tool.define("screenpipe_recall", {
  ...
  async execute(params, ctx) {
    const available = await isScreenpipeAvailable()
    const showWarning = !available && shouldShowWarning()
    const minutes = Number.isFinite(params.windowMinutes)
      ? Math.max(1, Math.min(7 * 24 * 60, Math.round(params.windowMinutes!)))
      : 90
