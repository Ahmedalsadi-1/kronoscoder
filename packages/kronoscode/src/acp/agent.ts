...
import type { AssistantMessage, Event, OpencodeClient, SessionMessageResponse } from "@kronoscode-ai/sdk/v2"
...
export namespace ACP {
  ...
  export class Agent implements ACPAgent {
    ...
    async initialize(params: InitializeRequest): Promise<InitializeResponse> {
      ...
      const authMethod: AuthMethod = {
        description: "Run `kronoscode auth login` in the terminal",
        name: "Login with kronoscode",
        id: "kronoscode-login",
      }
    ...
    private async processMessage(message: SessionMessageResponse) {
      ...
      for (const part of message.parts) {
        if (part.type === "tool") {
          ...
          // KronosCode stores files internally as { type: "file", url, filename, mime }.
  ...
  async function defaultModel(config: ACPConfig, cwd?: string) {
    ...
    const kronoscodeProvider = providers.find((p) => p.id === "kronoscode")
    if (kronoscodeProvider) {
      if (kronoscodeProvider.models["big-pickle"]) {
        return { providerID: "kronoscode", modelID: "big-pickle" }
      }
      const [best] = Provider.sort(Object.values(kronoscodeProvider.models))
      if (best) {
        return {
