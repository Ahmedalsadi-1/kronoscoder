...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for kronoscode.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal kronoscode sessions
...
kronoscode acp --cwd /path/to/project
```
...
```bash
KRONOSCODE_ENABLE_QUESTION_TOOL=1 kronoscode acp
```
...
- **Terminal Integration**: Full terminal support via kronoscode's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | kronoscode acp
```
...
### Mapping to KronosCode

ACP sessions map cleanly to kronoscode's internal session model:
