...
import { fn } from "@kronoscode-ai/util/fn"
  ...
  export async function fromDirectory(directory: string) {
    ...
    const data = await iife(async () => {
      ...
      if (dotgit) {
        ...
        let id = await Filesystem.readText(path.join(dotgit, "kronoscode"))
          .then((x) => x.trim())
          .catch(() => undefined)
        ...
        if (!gitBinary) {
          return {
            ...
            worktree: sandbox,
            sandbox: sandbox,
            vcs: Info.shape.vcs.parse(Flag.KRONOSCODE_FAKE_VCS),
          }
        }
        ...
        if (!id) {
          ...
          id = roots[0]
          if (id) {
            void Filesystem.write(path.join(dotgit, "kronoscode"), id).catch(() => undefined)
          }
        }
    ...
    if (Flag.KRONOSCODE_EXPERIMENTAL_ICON_DISCOVERY) discover(existing)
