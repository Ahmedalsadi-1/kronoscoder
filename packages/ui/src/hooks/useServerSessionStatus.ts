    ...
    try {
      ...
      for (const [sessionId, state] of Object.entries(statusSessions)) {
        ...
        const hasChanged =
          ...
          existing.activeTool !== state.metadata?.activeTool ||
          existing.workType !== state.metadata?.workType ||
          existing.runtimeTarget !== state.metadata?.runtimeTarget ||
        ...
        if (hasChanged) {
          ensureStatusesMap().set(sessionId, {
            ...
            activeTool: state.metadata?.activeTool,
            workType: state.metadata?.workType,
            runtimeTarget: state.metadata?.runtimeTarget,
          });
        }
      }
      ...
      for (const [sessionId, upstream] of Object.entries(upstreamStatuses)) {
        ...
        const hasChanged =
          ...
          existing.activeTool !== (typeof (upstream as { activeTool?: unknown }).activeTool === 'string'
            ? (upstream as { activeTool: string }).activeTool
            : undefined) ||
          existing.workType !== (typeof (upstream as { workType?: unknown }).workType === 'string'
            ? (upstream as { workType: string }).workType
            : undefined) ||
          existing.runtimeTarget !== (typeof (upstream as { runtimeTarget?: unknown }).runtimeTarget === 'string'
            ? (upstream as { runtimeTarget: string }).runtimeTarget
            : undefined);
        ...
        if (hasChanged) {
          ensureStatusesMap().set(sessionId, {
            ...
            next: upstream.next,
            activeTool: typeof (upstream as { activeTool?: unknown }).activeTool === 'string'
              ? (upstream as { activeTool: string }).activeTool
              : undefined,
            workType: typeof (upstream as { workType?: unknown }).workType === 'string'
              ? (upstream as { workType: string }).workType
              : undefined,
            runtimeTarget: typeof (upstream as { runtimeTarget?: unknown }).runtimeTarget === 'string'
              ? (upstream as { runtimeTarget: string }).runtimeTarget
              : undefined,
          });
        }
