...
import { kronoscodeClient, type RoutedOpencodeEvent } from '@/lib/kronoscode/client';
...
import type { Part, Session, Message } from '@kronoscode-ai/sdk/v2';
...
const BROWSING_TOOL_PATTERN =
  /^(browser_|kronoschamber_browser_|browseros_|mcp__.*__(browser_|browseros_|computer_use|computer[-_]?use|desktop|screen|screenshot|capture)|playwright|screenshot|capture|get_screen|take_screenshot|viewport_screenshot|webfetch|e2b)/i;

const partHasMediaAttachment = (part: Part): boolean => {
  if (part.type !== 'tool') {
    return false;
  }
  const attachments = ((part as unknown as { state?: { attachments?: unknown } }).state?.attachments);
  ...
  return attachments.some((entry) => {
    ...
    const candidate = entry as { mime?: unknown; url?: unknown };
...
const isBrowsingActivityPart = (part: Part): boolean => {
  if (part.type !== 'tool') {
    return false;
  }
  const tool = (part as unknown as { tool?: unknown }).tool;
  if (typeof tool === 'string' && BROWSING_TOOL_PATTERN.test(tool)) {
    return true;
  }
  return partHasMediaAttachment(part);
};
...
export const useEventStream = () => {
  ...
  const browsingEventPartsRef = React.useRef<Set<string>>(new Set());
  ...
  const handleEvent = React.useCallback((event: EventData) => {
    ...
    switch (event.type) {
      ...
      case 'session.status':
        {
          ...
          const activeTool =
            typeof statusInfo.activeTool === 'string'
              ? statusInfo.activeTool
              : typeof metadataObj?.activeTool === 'string'
                ? metadataObj.activeTool
                : undefined;
          const workType =
            typeof statusInfo.workType === 'string'
              ? statusInfo.workType
              : typeof metadataObj?.workType === 'string'
                ? metadataObj.workType
                : undefined;
          const runtimeTarget =
            typeof statusInfo.runtimeTarget === 'string'
              ? statusInfo.runtimeTarget
              : typeof metadataObj?.runtimeTarget === 'string'
                ? metadataObj.runtimeTarget
                : undefined;

          if (sessionId && statusType) {
            if (statusType === 'busy') {
             updateSessionStatus(sessionId, { type: 'busy', activeTool, workType, runtimeTarget }, 'sse:session.status');
              ...
              updateSessionStatus(sessionId, { type: 'idle', activeTool, workType, runtimeTarget }, 'sse:session.status');
            }
      ...
      case 'kronoschamber:session-status':
        {
          const sessionId = readStringProp(props, ['sessionId', 'sessionID']);
          ...
          if (sessionId && status) {
            ...
            if (status === 'busy') {
              updateSessionStatus(sessionId, { type: 'busy' }, 'sse:kronoschamber:session-status');
              ...
              updateSessionStatus(sessionId, {
                ...
                activeTool: typeof metadata.activeTool === 'string' ? metadata.activeTool : undefined,
                workType: typeof metadata.workType === 'string' ? metadata.workType : undefined,
                runtimeTarget: typeof metadata.runtimeTarget === 'string' ? metadata.runtimeTarget : undefined,
              }, 'sse:kronoschamber:session-status');
            } else {
              updateSessionStatus(sessionId, {
                type: 'idle',
                activeTool: typeof (props as { activeTool?: unknown }).activeTool === 'string'
                  ? (props as { activeTool: string }).activeTool
                  : undefined,
                workType: typeof (props as { workType?: unknown }).workType === 'string'
                  ? (props as { workType: string }).workType
                  : undefined,
                runtimeTarget: typeof (props as { runtimeTarget?: unknown }).runtimeTarget === 'string'
                  ? (props as { runtimeTarget: string }).runtimeTarget
                  : undefined,
              }, 'sse:kronoschamber:session-status');
            }
      ...
      case 'kronoschamber:agent-mode-task':
        {
          const taskPayload =
          ...
          const task = (taskPayload && typeof taskPayload === 'object')
            ...
            : null;
          const mode = readStringProp(task, ['mode']);
          const isBrowsingMode = mode === 'openbrowser' || mode === 'desktop-browser' || mode === 'browseros' || mode === 'e2b' || mode === 'user-desktop';
          if (!isBrowsingMode) {
            break;
          }

          const taskID = readStringProp(task, ['taskID', 'taskId']);
          const sessionID = readStringProp(task, ['sessionID', 'sessionId']);
          const taskDirectory =
            resolveSessionDirectoryForStatus(sessionID) ||
            normalizeDirectory(effectiveDirectory) ||
            normalizeDirectory(fallbackDirectory) ||
            null;
          ...
          if (taskDirectory) {
            ui.openContextRuntime(taskDirectory, taskID);
          }
        }
        break;
      ...
      case 'message.part.updated': {
        ...
        if (roleInfo === 'assistant' && isBrowsingActivityPart(messagePart)) {
          const rawPartID = (messagePart as unknown as { id?: unknown }).id;
          const partID = typeof rawPartID === 'string' && rawPartID.trim().length > 0
            ? rawPartID.trim()
            : `${messageId}:${(messagePart as unknown as { tool?: unknown }).tool || 'tool'}`;
          const eventKey = `${sessionId}:${partID}`;
          if (!browsingEventPartsRef.current.has(eventKey)) {
            browsingEventPartsRef.current.add(eventKey);
            if (browsingEventPartsRef.current.size > 600) {
              let index = 0;
              for (const key of browsingEventPartsRef.current) {
                browsingEventPartsRef.current.delete(key);
                index += 1;
                if (index >= 120) break;
              }
            }
            ...
            if (taskDirectory) {
              ui.openContextRuntime(taskDirectory, null);
            }
          }
        }
      ...
      case 'question.asked': {
        ...
        const toastKey = `${request.sessionID}:${request.id}`;
	        ...
	        // notifications are emitted server-side (see kronoschamber:notification)
      ...
      case 'kronoschamber:notification': {
