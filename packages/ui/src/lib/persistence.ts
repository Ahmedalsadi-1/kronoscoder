...
const persistToLocalStorage = (settings: DesktopSettings) => {
  ...
  if (typeof settings.screenpipeEnabled === 'boolean') {
    localStorage.setItem('screenpipeEnabled', settings.screenpipeEnabled ? 'true' : 'false');
  }
  if (typeof settings.screenpipeBaseUrl === 'string' && settings.screenpipeBaseUrl.length > 0) {
    localStorage.setItem('screenpipeBaseUrl', settings.screenpipeBaseUrl);
  }
  if (typeof settings.screenpipeAutoContext === 'boolean') {
    localStorage.setItem('screenpipeAutoContext', settings.screenpipeAutoContext ? 'true' : 'false');
  }
  if (typeof settings.openfangEnabled === 'boolean') {
    localStorage.setItem('openfangEnabled', settings.openfangEnabled ? 'true' : 'false');
  }
  if (typeof settings.openfangCliCommand === 'string' && settings.openfangCliCommand.length > 0) {
    localStorage.setItem('openfangCliCommand', settings.openfangCliCommand);
  }
  if (typeof settings.openfangAutoConfigureMcp === 'boolean') {
    localStorage.setItem('openfangAutoConfigureMcp', settings.openfangAutoConfigureMcp ? 'true' : 'false');
  }
  if (typeof settings.desktopQuickAssistEnabled === 'boolean') {
    localStorage.setItem('desktopQuickAssistEnabled', settings.desktopQuickAssistEnabled ? 'true' : 'false');
  }
  if (typeof settings.desktopQuickAssistTemplatesEnabled === 'boolean') {
    localStorage.setItem(
      'desktopQuickAssistTemplatesEnabled',
      settings.desktopQuickAssistTemplatesEnabled ? 'true' : 'false',
    );
  }
  if (typeof settings.desktopHoverAssistEnabled === 'boolean') {
    localStorage.setItem('desktopHoverAssistEnabled', settings.desktopHoverAssistEnabled ? 'true' : 'false');
  }
  if (typeof settings.desktopHoverAutoShowOnTaskSend === 'boolean') {
    localStorage.setItem(
      'desktopHoverAutoShowOnTaskSend',
      settings.desktopHoverAutoShowOnTaskSend ? 'true' : 'false',
    );
  }
  if (typeof settings.desktopHoverAlwaysOnTop === 'boolean') {
    localStorage.setItem('desktopHoverAlwaysOnTop', settings.desktopHoverAlwaysOnTop ? 'true' : 'false');
  }
...
export const syncDesktopSettings = async (): Promise<void> => {
  ...
  const applySettings = (settings: DesktopSettings) => {
    ...
    if (typeof window !== 'undefined') {
      window.dispatchEvent(new CustomEvent<DesktopSettings>('kronoschamber:settings-synced', { detail: settings }));
    }
  };
