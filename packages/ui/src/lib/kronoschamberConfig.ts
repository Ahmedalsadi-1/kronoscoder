 ...
 * Stores per-project settings in ~/.config/kronoschamber/<projectId>.json.
 * Migrates from legacy <project>/.kronoschamber/kronoschamber.json.
 */
...
const CONFIG_FILENAME = 'kronoschamber.json';
// LEGACY_PROJECT_CONFIG: legacy per-project config root inside repo.
const LEGACY_CONFIG_DIR = '.kronoschamber';
const USER_CONFIG_DIR_SEGMENTS = ['.config', 'kronoschamber'];
const USER_PROJECTS_DIR_SEGMENTS = ['.config', 'kronoschamber', 'projects'];
...
const resolveHomeDirectory = async (): Promise<string | null> => {
  ...
  // For user config (~/.config/kronoschamber), always use /api/fs/home in VSCode.
  if (!isVSCodeRuntime()) {
...
export async function readKronosChamberConfig(project: ProjectRef): Promise<KronosChamberConfig | null> {
  ...
  // 2) Migrate legacy <project>/.kronoschamber/kronoschamber.json.
  // LEGACY_PROJECT_CONFIG: migrate project-local kronoschamber.json -> ~/.config/kronoschamber/projects/<projectId>.json
...
export async function writeKronosChamberConfig(
  ...
  try {
    ...
    console.error('Failed to write kronoschamber config:', error);
    return false;
  }
}
