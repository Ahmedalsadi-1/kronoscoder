...
export const debugUtils = {
  ...
  findEmptyMessages() {
   ...
   showRetryHelp() {
    ...
    console.log('   __kronoscodeDebug.getLastAssistantMessage()\n');
    console.log('2. Find all empty messages in session:');
    console.log('   __kronoscodeDebug.findEmptyMessages()\n');
...
if (typeof window !== 'undefined') {
  (window as any).__kronoscodeDebug = debugUtils;
  if (streamDebugEnabled()) {
    console.log('[DEBUG] KronosCode Debug Utils loaded! Use window.__kronoscodeDebug in console');
    console.log('Available commands:');
    console.log('  __kronoscodeDebug.getLastAssistantMessage() - Get last assistant message details');
    console.log('  __kronoscodeDebug.getAllMessages(truncate?) - List all messages (truncate=true for short preview)');
    console.log('  __kronoscodeDebug.truncateMessages(messages) - Truncate long fields in messages array');
    console.log('  __kronoscodeDebug.getAppStatus() - Show app status snapshot');
    console.log('  __kronoscodeDebug.checkLastMessage() - Check if last message is problematic');
    console.log('  __kronoscodeDebug.findEmptyMessages() - Find all empty assistant messages');
    console.log('  __kronoscodeDebug.showRetryHelp() - Show instructions for handling empty responses');
    console.log('  __kronoscodeDebug.getStreamingState() - Get streaming state info');
    console.log('  __kronoscodeDebug.analyzeMessageCompletionConsistency(opts?) - Compare time.completed vs part timings');
    console.log('  __kronoscodeDebug.checkCompletionStatus() - Check completion status of last message');
  }
