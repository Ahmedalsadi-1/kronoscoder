...
import type { MultiRunLauncherPrefillContext } from '@/stores/useUIStore';
import { getWorktreeSetupCommands } from '@/lib/kronoschamberConfig';
import type { ProjectRef } from '@/lib/kronoschamberConfig';
...
interface MultiRunLauncherProps {
  ...
  /** Optional context metadata to include with prefill */
  initialContext?: MultiRunLauncherPrefillContext | null;
...
const buildContextPrefillText = (context?: MultiRunLauncherPrefillContext | null): string => {
  if (!context) {
    return '';
  }
  ...
  if (typeof context.source === 'string' && context.source.trim().length > 0) {
    lines.push(`source: ${context.source.trim()}`);
  }
  ...
  if (Array.isArray(context.paths) && context.paths.length > 0) {
...
const buildPromptPrefill = (prompt?: string, context?: MultiRunLauncherPrefillContext | null): string => {
  const basePrompt = typeof prompt === 'string' ? prompt.trim() : '';
  const contextBlock = buildContextPrefillText(context);

  if (basePrompt.length > 0 && contextBlock.length > 0) {
    return `${basePrompt}\n\n${contextBlock}`;
  }
...
export const MultiRunLauncher: React.FC<MultiRunLauncherProps> = ({
  ...
  const [prompt, setPrompt] = React.useState(() => buildPromptPrefill(initialPrompt, initialContext));
  ...
  React.useEffect(() => {
    const nextPrompt = buildPromptPrefill(initialPrompt, initialContext);
    if (!nextPrompt) {
      return;
    }
    setPrompt((prev) => (prev.trim().length > 0 ? prev : nextPrompt));
        ...
        <div className="chat-column py-6">
          <form onSubmit={handleSubmit} className="space-y-6" data-keyboard-avoid="true">
            ...
            <div className="space-y-2">
              ...
              {initialContext && (
                <p className="typography-micro text-muted-foreground">
                  Prefill context: {initialContext.source ? initialContext.source : 'external'}.
                </p>
              )}
              <Textarea
                id="prompt"
