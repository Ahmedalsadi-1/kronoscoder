...
import FileOpenChooser from '@/components/files/FileOpenChooser'; // Changed to default import
...
export const FilesView: React.FC<FilesViewProps> = ({ mode = 'full' }) => {
  ...
  const MD_VIEWER_MODE_KEY = 'kronoschamber:files:md-viewer-mode';
  ...
  // const handleRequestOpenFile = React.useCallback((node: FileNode) => { // Removed
  //   if (!root) return;
  //   setSelectedPath(root, node.path);
  //   addOpenPath(root, node.path);
  //   setPendingOpenFile(node);
  // }, [addOpenPath, root, setSelectedPath]);

  // const handleSelectOpenTarget = React.useCallback((target: FileOpenTarget) => { // Removed
  //   if (!pendingOpenFile || !root) {
  //     setPendingOpenFile(null);
  //     return;
  //   }

  //   if (target === 'side-panel') {
  //     openContextFile(root, pendingOpenFile.path);
  //     setPendingOpenFile(null);
  //     return;
  //   }
  ...
  //   void openDesktopPath(pendingOpenFile.path).then((ok) => {
  //     if (!ok) {
  //       toast.error('Unable to open file externally');
  //     }
  //   });
  //   setPendingOpenFile(null);
  // }, [openContextFile, pendingOpenFile, root, setActiveMainTab]);
      ...
      return (
        <li key={node.path} className="relative">
          ...
          <FileRow
            ...
            onSelect={handleSelectFile} // Original onSelect
  ...
  }, [childrenByDir, expandedPaths, handleSelectFile, selectedFile?.path, toggleDirectory, handleOpenDialog, handleRevealPath, canCreateFile, canCreateFolder, canRename, canDelete, canReveal, contextMenuPath, setContextMenuPath, isMobile, getFileStatus, getFolderBadge, root]);
  ...
  const blockWidgets = React.useMemo(() => {
    return buildCodeMirrorCommentWidgets({
      ...
      mapDraftToRange: (draft) => ({ startLine: draft.startLine, endLine: draft.endLine }),
