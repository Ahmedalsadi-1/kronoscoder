...
export const FlickeringGrid: React.FC<FlickeringGridProps> = ({
  ...
  const memoizedColor = useMemo(() => {
    // Helper to convert hex/named colors to rgba prefix
    const toRGBA = (color: string) => {
      ...
      if (!ctx) {
        return "rgba(0, 0, 0," // Default fallback
      }
      ctx.fillStyle = color
  ...
  useEffect(() => {
    ...
    let gridParams: { cols: number; rows: number; squares: Float32Array; dpr: number } | undefined
