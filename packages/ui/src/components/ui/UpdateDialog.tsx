...
const GITHUB_RELEASES_URL = 'https://github.com/btriapitsyn/kronoschamber/releases';
...
async function installWebUpdate(): Promise<{ success: boolean; error?: string }> {
  try {
    const response = await fetch('/api/kronoschamber/update-install', {
      method: 'POST',
...
export const UpdateDialog: React.FC<UpdateDialogProps> = ({
  ...
  const updateCommand = info?.updateCommand || 'kronoschamber update';
  ...
  const handleWebUpdate = useCallback(async () => {
    ...
    if (serverBack) {
      ...
      setWebUpdateState('error');
      setWebError('Server did not restart. Please refresh manually or run: kronoschamber restart');
    }
  }, []);
