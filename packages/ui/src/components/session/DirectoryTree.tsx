...
export const DirectoryTree: React.FC<DirectoryTreeProps> = ({
  ...
  React.useEffect(() => {
    ...
    return () => {
      cancelled = true;
      window.removeEventListener('kronoschamber:settings-synced', handleSettingsSynced);
    };
  ...
  const loadDirectory = React.useCallback(async (path: string): Promise<DirectoryItem[]> => {
    ...
    try {
      const filesystemEntries = await kronoscodeClient.listLocalDirectory(path);
      ...
      try {
        ...
        const tempClient = kronoscodeClient.getApiClient();
  ...
  const createDirectory = async () => {
    ...
    try {
      await kronoscodeClient.createDirectory(fullPath, { allowOutsideWorkspace: true });
