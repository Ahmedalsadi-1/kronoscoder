...
export const ProjectNotesTodoPanel: React.FC<ProjectNotesTodoPanelProps> = ({
  ...
  const handleSendToMultiRun = React.useCallback(
    (todoText: string) => {
      if (!projectRef) {
        return;
      }
      ...
      const prefillPrompt = [
        `Execute this project todo in parallel and compare outcomes:`,
        trimmedTodo,
        notesSummary ? `\nProject notes:\n${notesSummary}` : '',
      ]
      ...
      openMultiRunLauncherWithPrompt(prefillPrompt, {
        source: 'project-todo',
        tags: ['todo', 'project-notes'],
        paths: [projectRef.path],
      });
      toast.success('Todo opened in Multi-Run launcher');
      onActionComplete?.();
    },
    [notes, onActionComplete, openMultiRunLauncherWithPrompt, projectRef, routeToChat]
  );
                ...
                <li key={todo.id} className="flex items-center gap-1.5 px-2.5 py-1.5">
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end" className="w-56">
                      ...
                      <DropdownMenuItem onClick={() => handleSendToMultiRun(todo.text)}>
                        Open in Multi-Run launcher
                      </DropdownMenuItem>
                      <DropdownMenuItem
