      ...
      kronoscodeClient
        ...
        .catch(async () => {
          ...
          const looksLikeSdkWorktree =
            directory.includes('/kronoscode/worktree/') ||
            directory.includes('/.kronoscode/data/worktree/') ||
            directory.includes('/.local/share/kronoscode/worktree/');

          if (looksLikeSdkWorktree) {
            const ok = await kronoscodeClient.probeDirectory(directory).catch(() => false);
            if (ok) {
              setDirectoryStatus((prev) => {
                const next = new Map(prev);
                if (next.get(directory) === 'exists') {
