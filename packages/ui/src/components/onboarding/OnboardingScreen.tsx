...
const PATH_FIX_COMMAND = 'export PATH=\"$HOME/.local/bin:$PATH\"';
...
export function OnboardingScreen({ onCliAvailable }: OnboardingScreenProps) {
  ...
  const handleCopyPathFix = React.useCallback(async () => {
    const result = await copyTextToClipboard(PATH_FIX_COMMAND);
    if (result.ok) {
      setPathCopied(true);
    ...
    >
        ...
        <div className="mx-auto w-full max-w-xl pt-4">
          <div className="space-y-2">
            ...
            <div className="text-xs text-muted-foreground/70">
              Saves to <code className="text-foreground/70">~/.config/kronoschamber/settings.json</code> and reloads KronosCode configuration.
            </div>
          </div>
        </div>
      </div>
      ...
      {showHint && (
        <div className="absolute bottom-8 left-0 right-0 text-center space-y-1">
          ...
          <div className="pt-2">
            <Button type="button" size="sm" variant="outline" onClick={handleCopyPathFix}>
              {pathCopied ? 'Copied PATH command' : 'Copy PATH fix command'}
            </Button>
          </div>
        </div>
      )}
