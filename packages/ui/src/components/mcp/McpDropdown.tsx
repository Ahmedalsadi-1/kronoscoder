...
export const McpDropdownContent: React.FC<McpDropdownContentProps> = ({ active, className }) => {
  ...
  const toggleServer = React.useCallback(async (serverName: string, checked: boolean) => {
      ...
      <div className="max-h-64 overflow-y-auto py-1">
        {sortedNames.map((serverName) => {
          ...
          return (
            <div
              key={serverName}
              className="flex items-center justify-between gap-2 px-2 py-1.5 rounded-lg hover:bg-interactive-hover/50 cursor-pointer"
              onClick={() => void toggleServer(serverName, !isConnected)}
            >
              ...
              <div onClick={(event) => event.stopPropagation()} onPointerDown={(event) => event.stopPropagation()}>
                <Switch
                  ...
                  onCheckedChange={(checked) => void toggleServer(serverName, checked)}
                />
              </div>
            </div>
          );
        ...
        {sortedNames.length === 0 && (
          <div className="px-2 py-3 typography-ui-label text-muted-foreground text-center">
            Configure MCP servers in KronosCode settings.
          </div>
        )}
      </div>
    </div>
