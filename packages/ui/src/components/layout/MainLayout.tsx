...
import { KronosCodeStatusDialog } from '../ui/KronosCodeStatusDialog';
...
export const MainLayout: React.FC = () => {
    ...
    const {
        ...
        multiRunLauncherPrefillContext,
        autoSplitOnMedia,
        povPinned,
        povAutoSplitSuppressed,
    ...
    const autoSplitCollapseTimeoutRef = React.useRef<number | null>(null);
    ...
    const isRuntimeMediaWork = Boolean(
        agentWorkSnapshot &&
        (agentWorkSnapshot.work_type === 'browsing'
            || agentWorkSnapshot.work_type === 'mobile'
            || agentWorkSnapshot.has_media_activity)
    );

    React.useEffect(() => {
        const clearCollapseTimer = () => {
            if (autoSplitCollapseTimeoutRef.current !== null) {
                window.clearTimeout(autoSplitCollapseTimeoutRef.current);
                autoSplitCollapseTimeoutRef.current = null;
            }
        };

        clearCollapseTimer();

        if (!isDesktopApp || isMobile) {
            return clearCollapseTimer;
        }

        const isActiveWork = agentWorkSnapshot?.status === 'running' || agentWorkSnapshot?.status === 'retrying';
        const shouldAutoSplit = Boolean(autoSplitOnMedia && isActiveWork && isRuntimeMediaWork);

        if (shouldAutoSplit && !povAutoSplitSuppressed) {
            if (activeMainTab !== 'browser') {
                setActiveMainTab('browser');
            }
            if (layoutMode !== 'split') {
                setBrowserChatLayoutMode('split', { directory: directoryKey || null, makeDefault: !directoryKey });
            }
            if (!runtimeAutoSplitEngaged) {
                setRuntimeAutoSplitEngaged(true);
            }
            return clearCollapseTimer;
        }

        if (!isActiveWork) {
            if (povAutoSplitSuppressed) {
                setPovAutoSplitSuppressed(false);
            }

            if (runtimeAutoSplitEngaged && activeMainTab !== 'browser') {
                setRuntimeAutoSplitEngaged(false);
            }

            if (runtimeAutoSplitEngaged && !povPinned && activeMainTab === 'browser') {
                autoSplitCollapseTimeoutRef.current = window.setTimeout(() => {
                    setActiveMainTab('chat');
                    setRuntimeAutoSplitEngaged(false);
                    autoSplitCollapseTimeoutRef.current = null;
                }, 6000);
                        ...
                        <div className="absolute inset-0 z-10 bg-background header-safe-area">
                            <ErrorBoundary>
                                <MultiRunLauncher
                                    ...
                                    initialContext={multiRunLauncherPrefillContext}
