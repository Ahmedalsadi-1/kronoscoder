...
export const VSCodeLayout: React.FC = () => {
  ...
  React.useEffect(() => {
    ...
    void vscodeApi.executeCommand('kronoschamber.setActiveSession', currentSessionId, activeSessionTitle);
  ...
  React.useEffect(() => {
    ...
    window.addEventListener('kronoschamber:navigate', handler as EventListener);
    return () => window.removeEventListener('kronoschamber:navigate', handler as EventListener);
  }, []);
    ...
    const runBootstrap = async () => {
      ...
      try {
        const debugEnabled = (() => {
          ...
          try {
            return window.localStorage.getItem('kronoschamber_stream_debug') === '1';
          } catch {
            return false;
          }
        })();
