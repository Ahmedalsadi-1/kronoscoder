...
export const OpenInAppButton = ({ directory, className }: OpenInAppButtonProps) => {
  ...
  React.useEffect(() => {
    ...
    window.addEventListener('kronoschamber:settings-synced', handler);
    return () => window.removeEventListener('kronoschamber:settings-synced', handler);
  }, []);
  ...
  React.useEffect(() => {
    ...
    window.addEventListener('kronoschamber:app-ready', handler);
    ...
    window.addEventListener('kronoschamber:installed-apps-updated', updateHandler);
    const flag = (window as unknown as { __kronoschamberAppReady?: boolean }).__kronoschamberAppReady;
    if (flag) {
      void loadInstalledApps();
    }
    return () => {
      window.removeEventListener('kronoschamber:app-ready', handler);
      window.removeEventListener('kronoschamber:installed-apps-updated', updateHandler);
    };
