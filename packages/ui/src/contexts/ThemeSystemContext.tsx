...
export function ThemeSystemProvider({ children, defaultThemeId }: ThemeSystemProviderProps) {
  ...
  const [vscodeTheme, setVSCodeTheme] = useState<Theme | null>(() => {
    ...
    const existing = (window as unknown as { __KRONOSCHAMBER_VSCODE_THEME__?: Theme }).__KRONOSCHAMBER_VSCODE_THEME__;
  ...
  useEffect(() => {
    ...
    window.addEventListener('kronoschamber:vscode-theme', handleThemeEvent as EventListener);
    return () => window.removeEventListener('kronoschamber:vscode-theme', handleThemeEvent as EventListener);
  }, [isVSCode]);
  ...
  useEffect(() => {
    ...
    window.addEventListener('kronoschamber:settings-synced', handleSettingsSynced);
    return () => window.removeEventListener('kronoschamber:settings-synced', handleSettingsSynced);
  }, []);
