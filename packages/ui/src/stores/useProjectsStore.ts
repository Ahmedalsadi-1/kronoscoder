...
const getVSCodeWorkspaceProject = (): { projects: ProjectEntry[]; activeProjectId: string | null } | null => {
  ...
  const runtimeApis = (window as unknown as { __KRONOSCHAMBER_RUNTIME_APIS__?: { runtime?: { isVSCode?: boolean } } })
...
if (typeof window !== 'undefined') {
  window.addEventListener('kronoschamber:settings-synced', (event: Event) => {
