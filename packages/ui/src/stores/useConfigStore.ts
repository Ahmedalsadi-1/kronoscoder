...
import type { Provider, Agent } from "@kronoscode-ai/sdk/v2";
...
import { prewarmPredictiveSkills } from "@/lib/predictiveSkillsPrewarm";

const MODELS_DEV_API_URL = "https://models.dev/api.json";
const MODELS_DEV_PROXY_URL = "/api/kronoschamber/models-metadata";

const FALLBACK_PROVIDER_ID = "kronoscode";
const FALLBACK_MODEL_ID = "big-pickle";

interface KronosChamberDefaults {
    defaultModel?: string;
...
const fetchKronosChamberDefaults = async (): Promise<KronosChamberDefaults> => {
    try {
...
interface ConfigStore {
    ...
    // KronosChamber settings-based defaults (take precedence over agent preferences)
        ...
        persist(
            (set, get) => ({
                ...
                loadAgents: async (options) => {
                    ...
                    for (let attempt = 0; attempt < 3; attempt++) {
                        try {
                            // Fetch agents and KronosChamber settings in parallel
                            ...
                            // 1. Check KronosChamber settings for default agent
                            if (openChamberDefaults.defaultAgent) {
                             ...
                             // 1. Check KronosChamber settings for default model
                ...
                setAgent: (agentName: string | undefined) => {
                    ...
                    const activeDirectory = useDirectoryStore.getState().currentDirectory;
                    ...
                    if (agentName && typeof window !== "undefined") {
                        if (typeof activeDirectory === "string" && activeDirectory.trim().length > 0) {
                        ...
                        if (sessionStore) {
                            ...
                            const { currentSessionId, isKronosChamberCreatedSession, initializeNewKronosChamberSession, getAgentModelForSession } = sessionState;
                ...
                checkConnection: async () => {
                    ...
                    if (lastError) {
                        console.warn("[ConfigStore] Failed to reach KronosCode after retrying:", lastError);
                    }
