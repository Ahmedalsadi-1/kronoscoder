...
export async function scanSkillsRepository({
  ...
  const tempBase = await fs.promises.mkdtemp(path.join(os.tmpdir(), 'kronoschamber-skills-scan-'));
  ...
  try {
    ...
    // Root-level SKILL.md doesn't map cleanly to KronosCode's "skill name == folder name" convention.
    ...
    const worker = async () => {
      while (idx < uniqueSkillDirs.length) {
        ...
        if (!installable) {
          warnings.push('Skill directory name is not a valid KronosCode skill name');
        }
