...
const STORAGE_DIR = KRONOSCHAMBER_DATA_DIR;
const STORAGE_FILE = path.join(STORAGE_DIR, 'github-auth.json');
const SETTINGS_FILE = path.join(KRONOSCHAMBER_DATA_DIR, 'settings.json');
...
function writeJsonFile(payload) {
  ...
  // Atomic write so multiple KronosChamber instances can safely share the same file.
...
export function getGitHubClientId() {
  const raw = process.env.KRONOSCHAMBER_GITHUB_CLIENT_ID;
...
export function getGitHubScopes() {
  const raw = process.env.KRONOSCHAMBER_GITHUB_SCOPES;
