...
const DIFF_STATS_NEW_FILE_LIMIT = 400;
const DIFF_STATS_IGNORED_PREFIXES = [
  'Library/Containers/',
  'Library/Application Support/',
  '.zsh_sessions/',
  '.terminfo/',
  'Applications/',
  'Documents/iSimulator/',
  'Dropbox/',
];

const extractErrnoCode = (error) => {
...
const resolveCandidateDirectory = async (worktreeRoot, preferredName, explicitBranchName, primaryWorktree) => {
  ...
  for (const name of candidates) {
    ...
    const branch = `kronoschamber/${name}`;
...
export async function getStatus(directory) {
  ...
  let repoRoot = directoryPath;

  try {
    try {
      const topLevel = await git.raw(['rev-parse', '--show-toplevel']);
      const trimmedTop = String(topLevel || '').trim();
      if (trimmedTop) {
        repoRoot = normalizeDirectoryPath(trimmedTop);
      }
    } catch {
    ...
    const eligibleNewFiles = status.files.filter((file) => {
      if (!file || typeof file.path !== 'string') {
        return false;
      }
      const normalizedPath = file.path.replace(/\\/g, '/');
      if (DIFF_STATS_IGNORED_PREFIXES.some((prefix) => normalizedPath.startsWith(prefix))) {
        return false;
      }
    ...
    const limitedNewFiles = eligibleNewFiles.slice(0, DIFF_STATS_NEW_FILE_LIMIT);
    const newFileStats = await Promise.all(
      limitedNewFiles.map(async (file) => {
        const working = (file.working_dir || '').trim();
        const indexStatus = (file.index || '').trim();
        const statusCode = working || indexStatus;
        ...
        try {
          ...
          // New/untracked files can disappear between status collection and stat/read
          // (common in macOS container/cache directories). Ignore these transient misses.
          const code = extractErrnoCode(error);
    ...
    try {
      ...
      if (mergeHeadExists) {
        ...
        if (headSha) {
          const mergeMsg = await fsp.readFile(path.join(repoRoot, '.git', 'MERGE_MSG'), 'utf8').catch(() => '');
          mergeInProgress = {
            head: headSha,
    ...
    try {
      ...
      const rebaseMergeExists = await fsp.stat(path.join(repoRoot, '.git', 'rebase-merge')).then(() => true).catch(() => false);
      const rebaseApplyExists = await fsp.stat(path.join(repoRoot, '.git', 'rebase-apply')).then(() => true).catch(() => false);
      
      if (rebaseMergeExists || rebaseApplyExists) {
        const rebaseDir = rebaseMergeExists ? 'rebase-merge' : 'rebase-apply';
        const headName = await fsp.readFile(path.join(repoRoot, '.git', rebaseDir, 'head-name'), 'utf8').catch(() => '');
        const onto = await fsp.readFile(path.join(repoRoot, '.git', rebaseDir, 'onto'), 'utf8').catch(() => '');
