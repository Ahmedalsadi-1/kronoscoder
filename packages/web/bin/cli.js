...
EXAMPLES:
  kronoschamber                    # Start on default port 3000 (or a free port)
  kronoschamber --port 8080        # Start on port 8080
  kronoschamber serve --daemon     # Start in background
  kronoschamber --try-cf-tunnel    # Start with Cloudflare Quick Tunnel
  kronoschamber stop               # Stop all running instances
  kronoschamber stop --port 3000   # Stop specific instance
  kronoschamber status             # Check status
...
async function getPidFilePath(port) {
  ...
  return path.join(tmpDir, `kronoschamber-${port}.pid`);
}

async function getInstanceFilePath(port) {
  const os = await import('os');
  const tmpDir = os.tmpdir();
  return path.join(tmpDir, `kronoschamber-${port}.json`);
}
...
const commands = {
  async serve(options) {
    ...
    if (options.port !== 0) {
      ...
      if (existingPid && isProcessRunning(existingPid)) {
        ...
        console.error('Use "kronoschamber stop" to stop the existing instance');
        process.exit(1);
      }
    ...
    if (options.daemon) {
      ...
      const resolvedPort = await new Promise((resolve) => {
        ...
        child.on('message', (msg) => {
          if (settled) return;
          if (msg && msg.type === 'kronoschamber:ready' && typeof msg.port === 'number') {
            settled = true;
            clearTimeout(timeout);
            resolve(msg.port);
          }
  ...
  async stop(options) {
    ...
    try {
      ...
      const pidFiles = files.filter(file => file.startsWith('kronoschamber-') && file.endsWith('.pid'));

      for (const file of pidFiles) {
        const port = parseInt(file.replace('kronoschamber-', '').replace('.pid', ''));
        if (!isNaN(port)) {
          const pidFilePath = path.join(tmpDir, file);
          const pid = readPidFile(pidFilePath);

          if (pid && isProcessRunning(pid)) {
            const instanceFilePath = path.join(tmpDir, `kronoschamber-${port}.json`);
            runningInstances.push({ port, pid, pidFilePath, instanceFilePath });
          } else {

            removePidFile(pidFilePath);
            removeInstanceFile(path.join(tmpDir, `kronoschamber-${port}.json`));
          }
        }
      }
    } catch (error) {
  ...
  async restart(options) {
    ...
    if (instancesToRestart.length === 0) {
      ...
      console.log('Use "kronoschamber serve" to start a new instance');
      return;
    }
      ...
      if (runningInstances.length > 0) {
        ...
        for (const instance of runningInstances) {
          try {
            ...
            console.log(`  Run manually: kronoschamber serve --port ${instance.port} --daemon`);
          }
        }
      }
    } else {
