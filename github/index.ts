...
import { createOpencodeClient } from "@kronoscode-ai/sdk"
...
function createOpencode() {
  ...
  const proc = spawn(`kronoscode`, [`serve`, `--hostname=${host}`, `--port=${port}`])
...
function assertPayloadKeyword() {
  ...
  if (!body.match(/(?:^|\s)(?:\/kronoscode|\/oc)(?=$|\s)/)) {
    throw new Error("Comments must mention `/kronoscode` or `/oc`")
  }
}
...
async function assertOpencodeConnected() {
  ...
  if (!connected) {
    throw new Error("Failed to connect to kronoscode server")
  }
}
...
function useShareUrl() {
  return isMock() ? "https://dev.kronoscode.ai" : "https://kronoscode.ai"
}
...
async function getAccessToken() {
  ...
  let response
  if (isMock()) {
    response = await fetch("https://api.kronoscode.ai/exchange_github_app_token_with_pat", {
      method: "POST",
      headers: {
    ...
    const oidcToken = await core.getIDToken("kronoscode-github-action")
    response = await fetch("https://api.kronoscode.ai/exchange_github_app_token", {
      method: "POST",
      headers: {
...
async function getUserPrompt() {
  ...
  let prompt = (() => {
    const body = payload.comment.body.trim()
    if (body === "/kronoscode" || body === "/oc") {
      if (reviewContext) {
        return `Review this code change and suggest improvements for the commented lines:\n\nFile: ${reviewContext.file}\nLines: ${reviewContext.line}\n\n${reviewContext.diffHunk}`
      }
      return "Summarize this thread"
    }
    if (body.includes("/kronoscode") || body.includes("/oc")) {
      if (reviewContext) {
...
async function configureGit(appToken: string) {
  ...
  await $`git config --global user.name "kronoscode-agent[bot]"`
  await $`git config --global user.email "kronoscode-agent[bot]@users.noreply.github.com"`
}
...
function generateBranchName(type: "issue" | "pr") {
  const timestamp = new Date()
    ...
    .split("T")
    .join("")
  return `kronoscode/${type}${useIssueId()}-${timestamp}`
}
...
function footer(opts?: { image?: boolean }) {
  ...
  const image = (() => {
    ...
    return `<a href="${useShareUrl()}/s/${shareId}"><img width="200" alt="${titleAlt}" src="https://social-cards.sst.dev/kronoscode-share/${title64}.png?model=${providerID}/${modelID}&version=${session.version}&id=${shareId}" /></a>\n`
  })()
  const shareUrl = shareId ? `[kronoscode session](${useShareUrl()}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
