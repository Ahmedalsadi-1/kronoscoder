...
import { $ } from "bun"
import { createOpencode } from "@kronoscode-ai/sdk/v2"
...
export async function getLatestRelease(skip?: string) {
  const data = await fetch("https://api.github.com/repos/anomalyco/kronoscode/releases?per_page=100").then((res) => {
...
export async function getCommits(from: string, to: string): Promise<Commit[]> {
  ...
  const compare =
    await $`gh api "/repos/anomalyco/kronoscode/compare/${fromRef}...${toRef}" --jq '.commits[] | {sha: .sha, login: .author.login, message: .commit.message}'`.text()
  ...
  const log =
    await $`git log ${fromRef}..${toRef} --oneline --format="%H" -- packages/kronoscode packages/sdk packages/plugin packages/desktop packages/app sdks/vscode packages/extensions github`.text()
  ...
  for (const hash of hashes) {
    ...
    for (const file of files.split("\n").filter(Boolean)) {
      if (file.startsWith("packages/kronoscode/src/cli/cmd/")) areas.add("tui")
      else if (file.startsWith("packages/kronoscode/")) areas.add("core")
...
async function summarizeCommit(kronoscode: Awaited<ReturnType<typeof createOpencode>>, message: string): Promise<string> {
  console.log("summarizing commit:", message)
  const session = await kronoscode.client.session.create()
  const result = await kronoscode.client.session
    .prompt(
      {
        sessionID: session.data!.id,
        model: { providerID: "kronoscode", modelID: "claude-sonnet-4-5" },
        tools: {
          "*": false,
        },
        parts: [
...
export async function generateChangelog(commits: Commit[], kronoscode: Awaited<ReturnType<typeof createOpencode>>) {
  // Summarize commits in parallel with max 10 concurrent requests
  const BATCH_SIZE = 10
  const summaries: string[] = []
  for (let i = 0; i < commits.length; i += BATCH_SIZE) {
    const batch = commits.slice(i, i + BATCH_SIZE)
    const results = await Promise.all(batch.map((c) => summarizeCommit(kronoscode, c.message)))
...
export async function getContributors(from: string, to: string) {
  ...
  const toRef = to === "HEAD" ? to : to.startsWith("v") ? to : `v${to}`
  const compare =
    await $`gh api "/repos/anomalyco/kronoscode/compare/${fromRef}...${toRef}" --jq '.commits[] | {login: .author.login, message: .commit.message}'`.text()
...
export async function buildNotes(from: string, to: string) {
  ...
  const kronoscode = await createOpencode({ port: 0 })
  const notes: string[] = []

  try {
    const lines = await generateChangelog(commits, kronoscode)
    notes.push(...lines)
