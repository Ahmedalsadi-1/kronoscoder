...
const EXTENSION_NAME = "kronoscode"
...
async function main() {
  ...
  // Delete any existing branches for kronoscode updates
  ...
  const prResult =
    await $`gh pr create --repo ${UPSTREAM_REPO} --base main --head ${FORK_REPO.split("/")[0]}:${branchName} --title "Update ${EXTENSION_NAME} to v${cleanVersion}" --body "Updating KronosCode extension to v${cleanVersion}"`
