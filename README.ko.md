...
# 패키지 매니저
npm i -g kronoscode-ai@latest        # bun/pnpm/yarn 도 가능
scoop install kronoscode             # Windows
choco install kronoscode             # Windows
brew install anomalyco/tap/kronoscode # macOS 및 Linux (권장, 항상 최신)
brew install kronoscode              # macOS 및 Linux (공식 brew formula, 업데이트 빈도 낮음)
sudo pacman -S kronoscode            # Arch Linux (Stable)
paru -S kronoscode-bin               # Arch Linux (Latest from AUR)
mise use -g kronoscode               # 어떤 OS든
nix run nixpkgs#kronoscode           # 또는 github:anomalyco/kronoscode 로 최신 dev 브랜치
```
...
KronosCode 는 데스크톱 앱으로도 제공됩니다. [releases page](https://github.com/anomalyco/kronoscode/releases) 에서 직접 다운로드하거나 [kronoscode.ai/download](https://kronoscode.ai/download) 를 이용하세요.
...
KronosCode 에는 내장 에이전트 2개가 있으며 `Tab` 키로 전환할 수 있습니다.
...
[agents](https://kronoscode.ai/docs/agents) 에 대해 더 알아보세요.

### 문서

KronosCode 설정에 대한 자세한 내용은 [**문서**](https://kronoscode.ai/docs) 를 참고하세요.

### 기여하기

KronosCode 에 기여하고 싶다면, Pull Request 를 제출하기 전에 [contributing docs](./CONTRIBUTING.md) 를 읽어주세요.

### KronosCode 기반으로 만들기

KronosCode 와 관련된 프로젝트를 진행하면서 이름에 "kronoscode"(예: "kronoscode-dashboard" 또는 "kronoscode-mobile") 를 포함한다면, README 에 해당 프로젝트가 KronosCode 팀이 만든 것이 아니며 어떤 방식으로도 우리와 제휴되어 있지 않다는 점을 명시해 주세요.
...
- 100% 오픈 소스
- 특정 제공자에 묶여 있지 않습니다. [KronosCode Zen](https://opencode.ai/zen) 을 통해 제공하는 모델을 권장하지만, KronosCode 는 Claude, OpenAI, Google 또는 로컬 모델과도 사용할 수 있습니다. 모델이 발전하면서 격차는 줄고 가격은 내려가므로 provider-agnostic 인 것이 중요합니다.
- 기본으로 제공되는 LSP 지원
- TUI 에 집중. KronosCode 는 neovim 사용자와 [terminal.shop](https://terminal.shop) 제작자가 만들었으며, 터미널에서 가능한 것의 한계를 밀어붙입니다.
- 클라이언트/서버 아키텍처. 예를 들어 KronosCode 를 내 컴퓨터에서 실행하면서 모바일 앱으로 원격 조작할 수 있습니다. 즉, TUI 프런트엔드는 가능한 여러 클라이언트 중 하나일 뿐입니다.

---

**커뮤니티에 참여하기** [Discord](https://discord.gg/kronoscode) | [X.com](https://x.com/kronoscode)
